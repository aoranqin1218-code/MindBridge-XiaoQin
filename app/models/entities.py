...
class ChatSession(Base):
    ...
    # mapped_column(...)（列定义） - 真正定义数据库列的属性
    # Mapped[X] 管 Python 类型，mapped_column(...) 管数据库列属性——两者配合才算一个完整的 ORM 字段
    ...
    # relationship(...)（不是列，是对象引用） - 数据库里不存在这个列，它是纯 Python 层面的对象导航
    user: Mapped[UserAccount] = relationship(back_populates="sessions")

    # session.messages → 返回这个会话的所有 ChatMessage 列表
    # cascade 意味着删除 ChatSession 时，自动级联删除它下面的所有 ChatMessage
